...
Hint 2: store the command output in a file with an extension .html and open the created file with your browser.
...
Now that you've seen what an HTTP request looks like from the server's point of view, let's see what the response looks like from the client's point of view.  In particular, `curl` just prints out the content sent back from the server, but we'd like to see the server headers.  Try `curl --help` to see the help and verify that the command line `curl -i 'https://randomword.saasbook.info'` will display BOTH  the server's response headers AND then the response body.

<details><summary> Based on the server headers, what is the server's HTTP response code giving the status of the client's request, and what version of the HTTP protocol did the server use to respond to the client?</summary><p><blockquote>The first line tells us that HTTP 2 was used, and that the request succeeded with code 200.</blockquote></p></details>
<p></p>
...
<b>Remember to replace the </b> `FAKE-SERVER-URL-HERE` <b>with your url</b>.
...
<details><summary>An HTML form when submitted generates an HTTP `POST` request from the browser.  In order to reach your fake server, with what URL should you replace FAKE-SERVER-URL-HERE in the above file?</summary> <p><blockquote> <code>http://localhost:8081</code> <blockquote></p></details>
<p></p>
...
<details><summary>How is the information you entered into the form presented to the server?  What tasks would a SaaS framework like Sinatra or Rails need to do to present this information in a convenient format to a SaaS app written in, say, Ruby? </summary><p><blockquote>The form contents are presented as a long string of the form  <code>key1=value1&key2=value2&...keyN=valueN</code> where each key is the name of a form field and the values are <a href="https://en.wikipedia.org/wiki/Percent-encoding">URL-escaped</a>.  The server framework must pick apart the keys and values, un-escape the  values, and present the collection in some nice way, like a hash. </blockquote></p></details>
<p></p>
...
* Can you have more than one Submit button?  If you do, how does the server tell which one was clicked?  (Hint: experiment with the attribtues of the `<input>` tag.)
...
# Test Yourself!
Complete the [Self-Check Quiz:  HTTP and URIs](https://learn.sewanee.edu/d2l/le/content/46396/viewContent/435572/View) on Brightspace.  Note that this quiz is ungraded, but you will be presented with correct answers for any questions that you miss, and we will discuss frequently missed questions during our next class.